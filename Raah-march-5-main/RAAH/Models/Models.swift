...
enum AccentTheme: String, CaseIterable, Codable {
    ...
    var color: Color {
        switch self {
        case .snow:     return Color(white: 1.00)
        case .silver:   return Color(white: 0.78)
        case .stone:    return Color(white: 0.55)
        case .slate:    return Color(white: 0.36)
    ...
    var gradient: [Color] {
        switch self {
        case .snow:     return [Color(white: 1.00), Color(white: 0.90), Color(white: 0.78)]
        case .silver:   return [Color(white: 0.78), Color(white: 0.92), Color(white: 0.62)]
        case .stone:    return [Color(white: 0.55), Color(white: 0.68), Color(white: 0.42)]
        case .slate:    return [Color(white: 0.36), Color(white: 0.50), Color(white: 0.24)]
        case .charcoal: return [Color(white: 0.20), Color(white: 0.30), Color(white: 0.12)]
        }
    }
...
struct SpatialContext {
    ...
    func systemPromptFragment(now: Date, timeZone: TimeZone) -> String {
        ...
        if !pois.isEmpty {
            ...
            // Use effectiveIsOpenNow so sort reflects real-time status, not stale cached tag
            let sortedPOIs = pois.sorted { a, b in
                let aOpen = effectiveIsOpenNow(poi: a, now: now, timeZone: timeZone) ?? (a.tags["is_open_now"] == "true")
                let bOpen = effectiveIsOpenNow(poi: b, now: now, timeZone: timeZone) ?? (b.tags["is_open_now"] == "true")
            ...
            // Hoisted outside map — no need to allocate on every iteration
            let utilityPrimaryTypes: Set<String> = [
            ...
            let poiDescriptions = sortedPOIs.prefix(20).map { poi in
                ...
                if let primaryType = poi.tags["primary_type"], utilityPrimaryTypes.contains(primaryType) {
                ...
                } else if let isOpen = effectiveIsOpenNow(poi: poi, now: now, timeZone: timeZone) {
                    if isOpen {
                ...
                if let dist = poi.distance {
                    ...
                    if effectiveIsOpenNow(poi: poi, now: now, timeZone: timeZone) == true,
    ...
    /// Recomputes open/closed from opens_at + closes_at tags at the current time.
    /// This overrides the cached is_open_now tag, which can be up to 4 hours stale.
    /// Falls back to the cached tag if hour strings are missing or unparseable.
    private func effectiveIsOpenNow(poi: POI, now: Date, timeZone: TimeZone) -> Bool? {
        if let opensAtStr = poi.tags["opens_at"], let closesAtStr = poi.tags["closes_at"] {
            let fmt = DateFormatter()
            fmt.locale = Locale(identifier: "en_US_POSIX")
            fmt.timeZone = timeZone
            fmt.dateFormat = "h:mm a"
            guard let parsedOpen = fmt.date(from: opensAtStr),
                  let parsedClose = fmt.date(from: closesAtStr) else {
                return poi.tags["is_open_now"].map { $0 == "true" }
            }
            var cal = Calendar.current
            cal.timeZone = timeZone
            let today = cal.dateComponents([.year, .month, .day], from: now)
            func todayAt(_ parsed: Date) -> Date? {
                let t = cal.dateComponents([.hour, .minute], from: parsed)
                var c = DateComponents()
                c.year = today.year; c.month = today.month; c.day = today.day
                c.hour = t.hour; c.minute = t.minute; c.second = 0; c.timeZone = timeZone
                return cal.date(from: c)
            }
            guard let openDate = todayAt(parsedOpen),
                  var closeDate = todayAt(parsedClose) else {
                return poi.tags["is_open_now"].map { $0 == "true" }
            }
            // Handle midnight crossover (e.g. opens 10 PM, closes 2 AM)
