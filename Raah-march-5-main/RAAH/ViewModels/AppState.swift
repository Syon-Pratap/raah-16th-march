...
final class AppState {
    ...
    init() {
        ...
        self.accentTheme = AccentTheme(rawValue: defaults.string(forKey: "\(p)accent_theme") ?? "") ?? .silver
    ...
    private func handleToolCall(name: String, callId: String, args: [String: Any]) {
        ...
        case "search_nearby":
            Task {
                ...
                // Merge into nearbyPOIs so pre-arrival briefing can find them if user navigates there
                await contextPipeline.mergeSearchResults(results)
                if results.isEmpty {
                    realtimeService.sendToolResult(
                        callId: callId,
        ...
        case "play_music":
            Task {
                let query = args["query"] as? String ?? ""
                let result = await musicService.play(query: query)
                realtimeService.sendToolResult(callId: callId, result: result)
            }

        case "pause_music":
            musicService.pause()
            realtimeService.sendToolResult(callId: callId, result: "Music paused.")

        case "skip_song":
            Task {
                await musicService.skipToNext()
                realtimeService.sendToolResult(callId: callId, result: "Skipped to next song.")
            }

        case "call_phone_number":
            let number = args["phone_number"] as? String ?? ""
            let place = args["place_name"] as? String ?? "the place"
            let cleaned = number.components(separatedBy: CharacterSet(charactersIn: " ()-")).joined()
            if !cleaned.isEmpty, let url = URL(string: "tel://\(cleaned)") {
                DispatchQueue.main.async {
                    UIApplication.shared.open(url)
                }
                realtimeService.sendToolResult(callId: callId, result: "Opening dialer for \(place) (\(number)).")
            } else {
                realtimeService.sendToolResult(callId: callId, result: "No phone number available for \(place).")
            }
            HapticEngine.light()
    ...
    private func buildCurrentSystemPrompt() -> String {
        ...
        let timeZone = contextPipeline.currentContext?.timezone
            .flatMap { TimeZone(identifier: $0) } ?? TimeZone.current
        let calendarFragment = calendarService.systemPromptFragment(timeZone: timeZone)
    ...
    /// Called after a permission is granted mid-session (e.g. calendar) to refresh the AI's context.
    func refreshSystemPromptIfConnected() {
        pushSystemPrompt()
    }
