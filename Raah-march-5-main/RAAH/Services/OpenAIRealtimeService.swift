...
final class OpenAIRealtimeService: NSObject {
    ...
    /// Immediately stops the AI mid-speech and returns to listening.
    func cancelCurrentResponse() {
        guard isConnected else { return }
        // Stop audio playback immediately
        if playbackPlayer.isPlaying { playbackPlayer.stop() }
    ...
    private func setupPlaybackIfNeeded() {
        ...
        // Modest gain boost — keeps AI voice audible without triggering iOS headphone safety warning
        playbackBoost.globalGain = 6.0
        ...
        Task { @MainActor in
            ...
            case "response.done":
                ...
                if !lastResponse.isEmpty {
                    onResponseComplete?(lastResponse)
                }
                // Play a short chime so the user knows exactly when to speak,
                // then re-enable mic once the chime has cleared the room (~120ms).
                playListeningChime()
                Task { @MainActor [weak self] in
    ...
    private var toolDefinitions: [[String: Any]] {
        [
            ...
            [
                "type": "function",
                "name": "play_music",
                "description": "Search Apple Music and start playing a song, artist, album, or genre/mood. Use when user says 'play', 'put on', 'I want to listen to', etc. When you call this, say ONLY: 'Playing [what they asked for].' — nothing else.",
                "parameters": [
                    "type": "object",
                    "properties": [
                        "query": [
                            "type": "string",
                            "description": "What to play — e.g. 'Dua Lipa', 'Blinding Lights', 'chill jazz', 'workout hip hop'"
                        ]
                    ],
                    "required": ["query"]
                ]
            ...
            [
                "type": "function",
                "name": "pause_music",
                "description": "Pause or stop currently playing music. Use when user says 'pause', 'stop the music', 'turn it off', 'mute'. Say ONLY: 'Paused.'",
                "parameters": [
                    "type": "object",
                    "properties": [:] as [String: Any]
                ]
            ...
            [
                "type": "function",
                "name": "skip_song",
                "description": "Skip to the next song. Use when user says 'skip', 'next song', 'change this', 'skip this one'. Say ONLY: 'Skipped.'",
                "parameters": [
                    "type": "object",
                    "properties": [:] as [String: Any]
                ]
            ...
            [
                ...
                "description": "Open the phone dialer with a number pre-filled so the user can call a place with one tap. Use when the user says 'call', 'ring', 'phone', or 'dial' a place or number. When you call this tool, say ONLY: 'Calling [place name].' — nothing else, no explanations.",
                "parameters": [
                    "type": "object",
                    "properties": [
                        "phone_number": [
                            "type": "string",
                            "description": "The phone number to dial, e.g. '+919876543210' or '02223001234'"
                        ],
                        "place_name": [
                            "type": "string",
                            "description": "Name of the place being called, e.g. 'McDonald's'"
                        ]
                    ],
                    "required": ["phone_number", "place_name"]
                ]
            ],
            [
                "type": "function",
    ...
    /// Plays a short 880 Hz chime through the playback engine to signal "ready to listen".
    /// Uses the same audio path as AI speech so it respects the active audio session.
    private func playListeningChime() {
        guard playbackSetup, playbackEngine.isRunning else { return }

        let frequency: Double = 880.0
        let duration: Double = 0.10  // 100ms
        let sr = hardwareSampleRate > 0 ? hardwareSampleRate : 48000.0
        let frameCount = Int(sr * duration)
        let hwFormat = playbackEngine.mainMixerNode.outputFormat(forBus: 0)

        guard let buffer = AVAudioPCMBuffer(pcmFormat: hwFormat,
        ...
        for ch in 0..<Int(hwFormat.channelCount) {
            ...
            for i in 0..<frameCount {
                let t = Double(i) / sr
                // Short attack (5ms) + longer release (25ms) to avoid clicks
                let env = min(t / 0.005, 1.0) * min((duration - t) / 0.025, 1.0)
                data[i] = Float(sin(2.0 * .pi * frequency * t) * 0.18 * env)
            }
        }
