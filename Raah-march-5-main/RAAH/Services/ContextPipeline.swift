...
final class ContextPipeline {
    ...
    func fetchContext(for location: CLLocation, isInIndia: Bool) async {
        ...
        // Push context immediately — AI is responsive without waiting for Wikipedia
        await MainActor.run {
        ...
        // Wikipedia enrichment runs in the background — adds summaries silently
        // and re-injects the system prompt once complete (top 5 POIs only)
        let unenrichedPOIs = pois
        Task { [weak self] in
            guard let self else { return }
            let topPOIs = Array(unenrichedPOIs.prefix(5))
            let remainingPOIs = unenrichedPOIs.count > 5 ? Array(unenrichedPOIs.suffix(from: 5)) : []
            let enrichedTop = await enrichWithWikipedia(pois: topPOIs)
            let enrichedPOIs = enrichedTop + remainingPOIs
            await MainActor.run { [weak self] in
                guard let self else { return }
                nearbyPOIs = enrichedPOIs
                currentContext?.pois = enrichedPOIs
                onContextRefreshed?()
            }
        }
    }
    ...
    /// Merges live search_nearby results into nearbyPOIs so get_directions briefing can find them.
    @MainActor
    func mergeSearchResults(_ newPOIs: [POI]) {
        for poi in newPOIs {
            let poiLoc = CLLocation(latitude: poi.latitude, longitude: poi.longitude)
            let isDuplicate = nearbyPOIs.contains { existing in
                CLLocation(latitude: existing.latitude, longitude: existing.longitude)
                    .distance(from: poiLoc) < 50
            }
            if !isDuplicate { nearbyPOIs.append(poi) }
        }
    }
    ...
    func buildSystemPrompt(
        ...
        LANGUAGE: Always respond in English. Never switch languages unless the user explicitly \
        speaks to you in another language first.
        ...
        if let calendar = calendarFragment {
            prompt += """
            ...
            Whenever the user asks about going somewhere, what to do, where to eat, or asks for any recommendation — you MUST acknowledge the schedule. Always include a one-line note like: "You've got [Event] at [time], so you have about X hours" before or alongside your recommendation. Don't skip this even if there's no conflict.

            Hard rules:
            - Event under 30 min away: refuse anything that takes over 20 min.
            - Event 30–90 min away: state the time constraint before recommending, not after.
            - Before recommending any place: estimate walk time + time there. If it doesn't fit, say so.
            Reference durations: fast food = 20 min, café = 30 min, sit-down meal = 60 min, bar = 60+ min, museum = 90+ min.

            TODAY'S SCHEDULE above is real calendar data. Use it.
            """
        }
