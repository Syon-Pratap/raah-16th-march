...
struct SettingsView: View {
    ...
    private var musicSection: some View {
        GlassCard {
            VStack(spacing: RAAHTheme.Spacing.md) {
                ...
                GlassToggleRow(
                    "Play Music by Voice",
                    subtitle: musicGranted ? "Say 'play something' to start" : "Connect Apple Music",
                    icon: "music.note",
                    iconColor: .pink,
                    isOn: Binding(
                        get: { musicGranted },
                ...
                Text("Say \"play some jazz\", \"play Drake\", \"skip\", or \"pause\" — no app switching needed")
                    .font(RAAHTheme.Typography.caption())
                    .foregroundStyle(.tertiary)
            }
        }
        .onAppear {
            musicGranted = appState.musicService.isAuthorized
        }
    }
    ...
    private var calendarSection: some View {
        GlassCard {
            VStack(spacing: RAAHTheme.Spacing.md) {
                sectionTitle("CALENDAR AWARENESS")

                GlassToggleRow(
                    "Read Calendar",
                    subtitle: calendarGranted ? "Syncing today's events" : "Let AI see your schedule",
                    icon: "calendar",
                    iconColor: .blue,
                    isOn: Binding(
                        get: { calendarGranted },
                        set: { newValue in
                            if newValue {
                                Task {
                                    ...
                                    if granted {
                                        loadTodayEvents()
                                        appState.refreshSystemPromptIfConnected()
                                    }
                                    HapticEngine.selection()
                                }
                            } else {
                ...
                if calendarGranted {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if todayEvents.isEmpty {
                            ...
                            ForEach(todayEvents, id: \.startDate) { event in
                                HStack(spacing: 10) {
                    ...
                    Text("AI avoids suggesting things that conflict with your schedule")
                        .font(RAAHTheme.Typography.caption())
                        .foregroundStyle(.tertiary)
                }
            }
        ...
        .onAppear {
            calendarGranted = appState.calendarService.isAuthorized
            if calendarGranted { loadTodayEvents() }
        }
    }

    private func loadTodayEvents() {
        todayEvents = appState.calendarService.todaysEvents()
    }
    ...
    private let budgetOptions: [(label: String, value: String, hint: String)] = [
        ("Any", "", "No preference"),
